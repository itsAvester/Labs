...
### Averey Individual Written Section
\![Caption for the picture.\] (/path/to/image.png)
This photo is from this past winter break. My family went to Hawaii, and I had a mission to find one of these shirts.

What are the applications of neural networks, and why would you decide to implement them in a situation over other methods?

6 months after graduation, I would be happy with any job in data science. 5 years after graduation, I would ideally be working in a sports field or aerospace field doing data science.

I hope my greatest career accomplishment will be to create a large language model that achieves a complex task that was deemed unachievable before the model.

Further my understanding of the foundations of data science and programming in R Studio, gain experience collaborating with fellow data scientists in hopes it will roughly translate to the industry, and get more comfortable with sharing my understanding of these course topics with others.

One day, I would like to manage or start a small firm that can use data science as a tool for consulting or creating specific LLMs to a company's needs.

### Alice Individual Written Section
\![DB 2025\] (/path/to/image.jpeg)
This is me in front of a Deutsche Bahn (German) train during my trip to Europe last summer. I had taken so many DB trains that upon realizing the photo was taken on my privacy-minded dad's phone and thus had no location data, I had to look up pictures to verify this was Frankfurt airport station.

I want to apply statistical learning methods to my advocacy with data class this semester, where I've decided my project focus will be birth justice in Boulder. Given the US having the poorest birth outcomes of any high-resource country, I want to use interviews or questionaires with Boulder birth workers and parents to quantify variables in local maternal care that are highly linked with maternal and neonatal outcomes. This would include assessing the state of science-based stillbirth prevention measures, occurence and severity of obstetric violence and disrespectful care, access to diverse and midwifery-model providers, and education about maternal early warning signs in our community. 

I would like to have moved to Europe within six months of graduation, where I'm excited to live in a walkable community and spend more more time with my family in Germany. In five years, I hope to have gained a footing somewhere in professional statistics that could lead me closer to a higher impact dream job making patient-centered differences in a field like genetics, maternal health, or healthcare quality. 
In this course, I need to work on my time management and I'd like to grow confident with unstructured and other matrix-centered learning methods. 
...
misclass_rate(predicted.ys,testing[,3])
```
A k value of 3 resulted in a misclassification rate of 0.47. With k=7, having a misclassification rate of 0.44.
The misclassification rate with k=7 was lower by .03.
...
ggplot() +
  ...
  scale_fill_manual(values = c("orange", "cornflowerblue"), name = "Region") +
  scale_color_manual(values = c("darkorange", "blue"), name = "Actual Class") +
  labs(title = "KNN Decision Boundary (k=7)",
       subtitle = "Black line indicates the Bayes-style decision boundary") +
   geom_contour(data = predicted.gridxy, aes(x = X1, y = X2, z = Y +1),  
               breaks = 1.5, color = "black", size = 1) + 
theme_minimal()
```
Q1: We want to use metrics from a college to predict if a student will likely graduate. We want a tool for universities to be able to preemptively reach out to students and let them know they are potentially on a bad path before it is too late. This could raise a university's graduation rate, which in turn helps the students who attend the university. The graduation prediction will take into account a student's GPA percentile and the percentage of classes they attend. The data would be collected by students opting in to giving their data via an email sent to the entire student body. The data would likely show that high GPA and attendance are very good predictors of if a student will graduate. By creating a decision boundary using K-nearest-neighbors and misclassification metrics to find the best k value for the model, an ideal model, given these metrics, could be found. This is a common technique and could be easily explained to those opting into the study as well as those viewing the report.

Q2: Y=If a student who attends college graduates. X1= percentage of classes attended. X2= percentile of GPA of a student after the first semester. With k values of 3 and 7, the misclassification rates were respectively 0.47 and 0.44.

Q3: The results show that our k-nearest-neighbors with a k value of 3, the method used, only correctly guessed 53% of the students' graduation based on their GPA percentile and the percentage they attended all classes. When using a k value of 7, the misclassification rate was 44%. This means that the method correctly predicted if a student would graduate 56% of the time. With these models, I would not be confident enough in the models with these metrics to publish this for university use. Ethically, this could be problematic as universities might use these metrics for a different, unintended use, such as putting fewer resources into students who are predicted not to graduate. This model also misclassified 44% at best, which ethically could be problematic, as it could cause unintentional stress to students who are predicted to fail, as the model could very likely be wrong and could lead to a student putting less effort into school or dropping out preemptively. 
...
```{r}
knn.pred5 <- knn(training, testing, Y, k = 5)
table(knn.pred5, Y)
mean(knn.pred5 == Y)

knn.pred5 <- knn(training, testing, Y, k = 1)
table(knn.pred5, Y)
mean(knn.pred5 == Y)
```
```{r}
# plotting k = 1
grid <- expand.grid(X1 = seq(0, 1, length = 200), X2 = seq(0, 1, length = 200)) #initializing for plotting 
grid$pred1 <- knn(training[,1:2], #extracts X1 & X2 
                  grid, 
                  as.factor(training[,3]), #extracts Y
                  k = 1)
ggplot() + 
  geom_tile(data = grid, aes(X1, X2, fill=pred1, alpha = .1)) +
  geom_point(data = testing, aes(X1, X2, color = as.factor(Y))) + 
  labs(title = "K = 1 KNN Decision Boundaries")
```

```{r}
# plotting k = 5
grid <- expand.grid(X1 = seq(0, 1, length = 200), X2 = seq(0, 1, length = 200)) #initializing for plotting 
grid$pred5 <- knn(training[,1:2], #extracts X1 & X2 
                  grid, 
                  as.factor(training[,3]), #extracts Y
                  k = 5)
ggplot() + 
  geom_tile(data = grid, aes(X1, X2, fill=pred5, alpha = .1)) +
  geom_point(data = testing, aes(X1, X2, color = as.factor(Y))) + 
  labs(title = "K = 5 KNN Decision Boundaries")
```

Q1: Given the low visual correlation between the response and predictors and a classification error rate greater than 50%, Y could represent the true diagnosis rate of an extremely rare condition upon invasive testing amongst subjects who *tested positive for the condition initially on a noninvasive screening*. X1 and X2 are normalized continuous numeric predictors of health correlated with occurence of the condition, such as red and white blood cell count in the case of predicting rare lymphoma. 
Q2: The misclassification rate for k = 1 was 0.55 and 0.51 for k = 5, meaning that positive initial lymphoma screening were false positives 45% and 49% of the time respectively. This indicates the initial screening test has low specificity, which is a sign of poor performance and not tolerated in test design for a more common condition (ex. prenatal tests for most chromosomal abnormalities). But a low true diagnosis rate is a statistical inevitability for screening rarer conditions (ex. prenatal testing for microdeletions). 
(transitioning example to prenatal testing, which I did not use the entire time due to not knowing any realistic predictor variables in the context of raw DNA analysis, let alone continuous numeric ones, but I'm in that class this semester!) 
Q3: Patient-facing prenatal testing is already rife with misunderstanding, so given this context (all individuals appearing on the visual are those who recieved a positive prenatal screening for a certain condition, where the Y results correspond to their true diagnosis after invasive testing), this method and visual would need to be explained carefully to patients and providers. Because invasive testing in this context (amniocentesis) is unpleasant and carries a number of risks, the audience needs to understand how dramatically the rarity of a condition could impact their decisions. This visual could denote either an ineffective test for a more common condition that should be phased out, or it could be the inevitable numeric result of screening for a very rare condition, which leads to different decisions for subsequent invasive testing. 
