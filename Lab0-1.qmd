...
pred10k <- knn(train=training[,1:2], test=testing[,1:2], cl=training[,3], k=10)

#Calculate Miscalculation rates for k=2 and k=10
truth <- testing[[3]]
pred2kn <- as.numeric(pred2k)
pred10kn <- as.numeric(pred10k)
...
misclass_rate(pred10kn, testing[,3])
```
It seems that k=10 yielded a better result with a misclassification rate of 0.64 over the 0.68 misclassification rate with k=2.
